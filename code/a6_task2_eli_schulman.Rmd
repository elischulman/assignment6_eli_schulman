---
title: "Survey Responses - Political and Environmnetal Identifications"
author: "Eli Schulman"
date: "11/22/2021"
...
There is a significant association between one's political party and whether they consider themselves an environmentalist ($\chi$^2^(`r pol_env_tidy$parameter`) = `r round(pol_env_tidy$statistic, 2)`. *p* `r if(pol_env_tidy$p.value < .001){"< .001"}else{pol_env_tidy$p.value}`). Of conservatives surveyed, only `r pol_env[1,2]` self-identified as environmentalists, while `r pol_env[1,3]` responded that they were not environmentalists and `r pol_env[1,4]` responded as "unsure". Alternatively, of librals surveyed, `r pol_env[2,2]` self-identified as environmentalists, while `r pol_env[2,3]` and `r pol_env[2,4]` responded as "no" or "unsure" to the question.

***NEED TO EXPLAIN THE RESULTS MORE. NOT IN STATS LINGO. TALK ABOUT PERCENTAGES. QUESTION: THE PERCENTAGES RELATE TO THE ROW TOTAL (I.E. 54% OF CONSERVATIVES), BUT NOT THE COLUMN TOTAL (I.E. X% OF ENVIRONMENTALISTS). IS THAT THE LIMITATION? ***
